...
async function main() {
    ...
    console.log(`[mempool-watch] Starting (chains=${options.chains.map((c) => c.id).join(',')} interval=${options.intervalMs}ms)`);
    while (true) {
        await Promise.all(options.chains.map(async (chain) => {
...
async function captureSnapshot(chain, provider, history) {
    const latest = await provider.getBlockNumber();
    const pending = await provider.send('eth_getBlockByNumber', ['pending', true]);
    if (!pending)
        return null;
    const transactions = Array.isArray(pending.transactions)
        ? pending.transactions
        : [];
    const pseudoHeight = pending.number && typeof pending.number === 'string'
        ? parseInt(pending.number, 16)
        : latest + 1;
    const timestampSeconds = pending.timestamp && typeof pending.timestamp === 'string'
    ...
    // eslint-disable-next-line no-console
    console.log(`[mempool-watch] ${chain.id} tx=${snapshot.txCount} avgGas=${snapshot.avgGasPriceGwei} gwei score=${snapshot.anomalyScore} tags=${snapshot.tags.slice(0, 3).join(',')}`);
    return snapshot;
}
function computeGasStats(transactions) {
    ...
    return {
        avgGwei: Number(avg / 1000000000n),
        maxGwei: Number(max / 1000000000n),
    };
}
function computeValueStats(transactions) {
    if (transactions.length === 0) {
        return { totalEth: 0 };
    }
    let total = 0n;
...
function deriveInsights(args) {
    ...
    const anomalyScore = Number(Math.min(txScore * 0.3 + gasScore * 0.3 + valueScore * 0.2 + tagScore + trendScore(deltaGas, deltaTx), 1).toFixed(2));
    const highlights = [];
    if (args.avgGasPriceGwei > 60)
        highlights.push('High gas');
    if (args.txCount > 200)
        highlights.push('Heavy throughput');
    if (args.tags.some((tag) => tag.includes('BRIDGE')))
        highlights.push('Bridge activity');
    if (args.tags.some((tag) => tag.includes('DEX')))
        highlights.push('DEX flow');
    if (highlights.length === 0)
        highlights.push('Normal');
    return { anomalyScore, highlights, deltaTx, deltaGas, deltaValue };
}
function trendScore(deltaGas, deltaTx) {
...
function recordLine(filePath, line) {
    try {
...
function generatePrediction(snapshot, history) {
    const tags = new Set();
    const reasons = [];
    ...
    const confidence = clamp(0.2 + snapshot.anomalyScore * 0.6 + Math.max(0, snapshot.deltaGas) * 0.01, 0, 1);
...
function estimateEtaSeconds(snapshot, history) {
    const previous = history[0];
    const cadence = previous && snapshot.fetchedAt !== previous.fetchedAt
...
function buildOptions() {
    const chainIds = process.env.MEMPOOL_CHAINS?.split(',')
        .map((c) => c.trim())
        .filter(Boolean) ?? CHAINS.map((c) => c.id);
    const chains = CHAINS.filter((chain) => chainIds.includes(chain.id));
    const intervalMs = Number.parseInt(process.env.MEMPOOL_INTERVAL_MS ?? '5000', 10) || 5000;
