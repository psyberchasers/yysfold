  ...
  `);
    ...
    rows.forEach((row) => {
        try {
            ...
            const peakDensity = hotzones.reduce((acc, hz) => Math.max(acc, Number(hz?.density ?? 0)), 0);
            const avgDensity = hotzones.length > 0
                ? hotzones.reduce((acc, hz) => acc + Number(hz?.density ?? 0), 0) /
                    hotzones.length
                : 0;
            ...
            insert.run(row.chain, row.height, timestampSeconds, hotzones.length, peakDensity, avgDensity, JSON.stringify(Array.from(new Set(tags ?? []))), behavior.dexGasShare, behavior.nftGasShare, behavior.lendingVolumeWei, behavior.bridgeVolumeWei, behavior.highFeeTxCount, behavior.dexTxCount, behavior.nftTxCount, behavior.lendingTxCount, behavior.bridgeTxCount, behavior.dominantFlow ?? null, JSON.stringify(behavior.topContracts ?? []), residualStats.average, residualStats.max, residualStats.p95);
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `);
            hotzones.forEach((hotzone, index) => {
                hotzoneStmt.run(row.chain, row.height, timestampSeconds, hotzone.id ?? `hotzone-${index}`, Number(hotzone.density ?? 0), Number(hotzone.radius ?? 0), JSON.stringify(hotzone.center ?? []), JSON.stringify(hotzone.semanticTags ?? []));
            });
...
function normalizeBehaviorMetrics(raw) {
    return {
...
function extractResidualValues(summary) {
    if (!summary)
        return [];
    if (Array.isArray(summary.pqResiduals)) {
        return summary.pqResiduals.map((value) => Number(value ?? 0));
    }
    if (Array.isArray(summary.pqResiduals?.values)) {
        return summary.pqResiduals.values.map((value) => Number(value ?? 0));
    }
    if (Array.isArray(summary.pqCode?.residuals)) {
        return summary.pqCode.residuals.map((value) => Number(value ?? 0));
    }
    return [];
}
