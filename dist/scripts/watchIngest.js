...
async function main() {
    ...
    console.log(`[watch-ingest] Starting watcher (chains=${options.chains.join(',')} batch=${options.batchSize} interval=${options.intervalMs}ms)`);
    let failureCount = 0;
...
function ingestCommand(options) {
...
async function runAtlasBuild(env) {
...
function spawnAndWait(label, args, env) {
...
function shouldBuildAtlas(lastBuild, intervalMs) {
    if (intervalMs <= 0) {
        return false;
    }
    if (lastBuild === 0) {
