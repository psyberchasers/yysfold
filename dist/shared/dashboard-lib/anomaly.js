...
export function computeAnomalyScore({ hotzones = [], pqResidualStats, tagVector = [], } = {}) {
    ...
    const scoreRaw = 0.5 * densityComponent.component +
...
function computeDensityComponent(hotzones = []) {
    if (!hotzones.length) {
...
function computeResidualComponent(stats) {
...
function computeTagComponent(tags = []) {
    if (!tags.length)
        return { component: 0, detail: 'No tags' };
