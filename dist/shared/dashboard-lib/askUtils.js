export const KEYWORD_MAP = [
    ...
    { regex: /avax|avalanche/i, tags: ['BRIDGE_ACTIVITY'] },
    ...
    { regex: /hotzone|density|fingerprint|cluster/i, tags: ['DEX_ACTIVITY'] },
    { regex: /arbitrage|mev/i, tags: ['DEX_ACTIVITY', 'HIGH_FEE'] },
...
export function inferTags(question) {
    ...
    words.forEach((word) => {
        if (word.length < 3)
            return;
        if (word === 'TECH' || word === 'TECHNOLOGY')
            tags.add('SECTOR_TECH');
        if (word === 'FINANCE' || word === 'BANK')
            tags.add('SECTOR_FINANCE');
        if (word === 'EMERGING' || word === 'EMERGING_MARKETS')
            tags.add('SECTOR_EMERGING_MARKETS');
        if (word.includes('SAFE'))
            tags.add('RISK_LOW');
        if (word.includes('ALERT'))
            tags.add('AML_ALERT');
    });
    return Array.from(tags);
}
export function dedupeBlocks(blocks, limit) {
    const seen = new Set();
    const out = [];
...
export function buildSummary(question, tags, sample) {
