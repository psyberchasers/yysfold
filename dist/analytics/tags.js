import { LENDING_CONTRACT_ADDRESSES, normalizeAddress as normalizeSharedAddress, } from '../shared/contracts/index.js';
...
export function deriveRawBlockTags(raw) {
    const tags = new Set();
...
function makeAddressSet(addresses) {
    return new Set(addresses.map((addr) => addr.toLowerCase()));
}
function addStringTag(value, prefix, tags) {
...
function addAssetTag(value, tags) {
    if (typeof value !== 'string' || value.length === 0)
        return;
    const normalized = value
        .trim()
...
function addSideTag(value, tags) {
...
function addRiskTags(score, tags) {
...
function addLiquidityTags(score, tags) {
    const numeric = Number(score);
    if (!Number.isFinite(numeric))
        return;
    if (numeric <= 0.3) {
...
function addSpreadTags(spread, tags) {
